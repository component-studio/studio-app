...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-zinc-100 h-full flex items-stretch">
        <div class="w-64 bg-zinc-100 fixed h-screen">
            ...
            <?php
                $dir = resource_path('/views/components/studio');
                $files = scandir($dir);
            ...
            @foreach($components as $component)
                <a href="/components?component={{ $component }}" class="@if($activeComponent == $component){{ 'bg-zinc-200 font-semibold' }}@else{{ 'font-normal' }}@endif px-5 block py-1 text-sm text-gray-600 hover:text-gray-800">{{ $component }}</a>
            @endforeach

        </div>

        <div class="pl-64 pt-2 bg-zinc-100 flex w-full h-screen items-stretch justify-stretch">
            <div class="w-full h-full bg-white rounded-tl-2xl">
                {{ $slot }}
            </div>
        </div>
    </body>
